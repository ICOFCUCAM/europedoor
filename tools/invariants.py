        ...
        "invariants": {
            ...
            "css.font_sizes": {
                ...
                "why": "A ceiling, not a target. The sibling repository measures "
                       ...
                       "AND 25 FOR THE DESTINATION'S OWN NAME ON THE "
                       "ARRIVAL BAND, which is the phone-head case one "
                       "level up and was measured the same way. The copy "
                       "column's h1 is 177px at 834, 211 at 1280 and 223 "
                       "from 1520 up — it never passes 239 at any width — "
                       "while `--ed-display-2` keeps growing to 76px, so "
                       "the word overran its column by 1.40x at 834 and "
                       "2.02x at 1920 and 129 of the 319 destinations set "
                       "their own name broken in half. A band-wide display "
                       "step is the wrong shape for one word in a column "
                       "that stops growing: this clamp caps where the band "
                       "caps. The two existing clamps that cap at --t-5xl "
                       "rise at 5.2vw and were measured failing at 834, and "
                       "--ed-display-3, which fits, is the size of a list "
                       "row — a destination name the size of the rows "
                       "beside it is the argument this same entry already "
                       "makes against --t-2xl for the two leads. Both ends "
                       "are existing STEPS of the scale (--t-2xl, --t-5xl); "
                       "what is new is the rate between them. 129 to 0, "
                       "measured on all 319 at six widths from 390 to 2560. "
            ...
            "js.enhancements": {
                ...
                "why": "1 -> 2 FOR THE ATLAS STAGE, AND THE SECOND ONE IS "
                       "ON THE HOMEPAGE, which until now loaded no "
                       "JavaScript at all — its only `<script>` was the "
                       "inert JSON-LD block. The band is scroll-native by "
                       "construction: the stage is sticky, the nine corners "
                       "are real blocks, and each card pins in the panel "
                       "slot by the stylesheet alone, so a reader with no "
                       "JavaScript gets the nine as a list and loses "
                       "nothing the page says. What no selector can do is "
                       "reach from a scrolled block to a `<text>` in the "
                       "figure above it, and *the continent answers the "
                       "reading* is the band's whole argument. 69 lines, "
                       "one IntersectionObserver, no fetch, no storage, no "
                       "client state — an enhancement by this site's own "
                       "test rather than by its file name, which is the "
                       "distinction the first version of this check had "
                       "backwards. A ROTATOR WAS REFUSED ON THIS PAGE ONCE "
                       "and the refusal still holds: that one would have "
                       "cross-faded one country for a register holding "
                       "photographs of one featured country, which is a "
                       "decoration bought with the page's first script. "
                       "This is the mechanism a band asked for by the "
                       "owner is composed of.\n"
                       "Scripts that do neither and work on markup already "
                       "in the page."},
            ...
            "safety.img_tags": {
                ...
                "kind": "exact",
                "why": "AND 3272 -> 3273 WHEN THE CROSSING DREW THE CROSSING. That band's subject is movement and it drew none — a headline, two figures and three rows, with the ordered line across the continent nowhere on it. It is two columns now, and the right one is the lead route drawn over a photograph of one of its own STOPS: a stop no row is already drawing, because one record one picture is a rule about a record and one picture twice on one screen is a different fault with the same shape. One `<img>`, from the register, nothing acquired. AND 3268 -> 3272 WHEN THE HOMEPAGE DREW ALL NINE CORNERS. Four `<img>` on one band, which is the whole of this move: plate 03 promised `each corner of the continent` in its heading and `picks[:5]` drew five. This row moving is the signal that licensed imagery arrived, and it is moving here because imagery the register already held reached a surface that was capped below its own claim. AND /countries SPENT THE NINE MACRO PHOTOGRAPHS AND TEN APERTURES, 1196 -> 1205. The register holds a photograph of every one of the fifty countries, one of each of the nine macro regions and the index's own hero — sixty pictures relevant to this page — and the page drew ONE. The nine macro photographs each appeared on exactly one page, their own, while the band whose whole subject is those nine regions drew none of them. Nine came on here; the ten country apertures are SVG `<image>` rather than `<img>` and this figure does not count them. Nothing was acquired. AND /stories SPENT THE SEVEN IT ALREADY HELD, "
            ...
            "weight.home_kb": {
                ...
                "kind": "ceiling",
                "why": "184 -> 192 FOR THE GROUND'S OWN HEIGHT AND A FRAME FITTED TO EUROPE, AT THE OWNER'S DIRECTION. `atlas_register` own comment refused relief on this band in as many words — *no water, no relief, no rivers, no lakes, no photograph and no dusk* — and that refusal held twice, once against four upgrades proposed in one session. It is the owner's to overrule and he has, and the one thing that gives a stone continent depth without adding a claim is the height of the ground: **a band claims only height, which is the one thing the elevation model measures**, so nothing is invented, which is what separates it from the water, the rivers and the photograph the same paragraph refuses and that stay refused. The settings were RENDERED AND LOOKED AT rather than picked — four built, injected into the real page and photographed at 1440: three bands at 3.5 units 15.7 KB, the same at 4.5 10.8, two bands at 4.5 4.4, three at 6.0 6.6 — and the two cheap ones are a layer that ships and cannot be seen, the exact measurement that removed the terrain second strength. Itemised on the shipped page, plate 05 drawing goes 45,543 bytes to 54,210: terrain 0 to **13,012**, the ground beyond 11,079 to **6,175** because the frame is fitted to the countries this atlas writes about rather than bled wide, the names 1,815 to 2,764 because eleven became seventeen, and the land 32,181 to 31,789. THE FRAME IS THE OWNER'S OTHER TWO INSTRUCTIONS AND THEY PAY FOR EACH OTHER: *the map should take only 2/3 and extend to the right* and *Russia is not supposed to be in Europe but it covers a bigger place than Europe, the focus is Europe*. `atlas_frame` leaves the CUT countries out of the fit, so the window is Europe minus the fragment of Russia this dataset holds, and five kilobytes of ground beyond come off with it. AND THE SOUTH GETS THE AIR: this frame has no visible edge, so land ending on a ruled line is the /plan context-land slab arrived at from below, measured at 1440 as Tunisia and Anatolia cut into two straight-edged wedges 730 pixels down the stage. `ATLAS_FRAME_PAD_S` is 0.20 against 0.05 on the other three and `ATLAS_GROUND_FADE` hands what is left to the water in five smoothstep stops, masked on the ground alone because a masked group hit-tests as ONE region and the land carries fifty links. It also shortens the dead band under the drawing from 235 pixels to 144 at 1280x900 without the continent losing a unit. AND 181 -> 184 FOR THE DOOR ON WHAT IS OPEN. The band says `One continent. Fifty doors.` and drew fifty country shapes with nothing on them that is a door — the /themes fault, a page describing shapes its own closing sentence is about and drawing none. Fifty arches were prototyped in Chromium at 6, 9, 13 and 16 units and photographed at 1x and 2x BEFORE any of this was written, and they are refused on the measurement: at the size that fits the smallest country it is a texture, and at the size that resolves into an arch it does not fit — Luxembourg largest ring spans 8 units and a mark needs about 13 to read — so forty-four at one weight draw a second frontier network over the first, which is `a signature applied to everything is wallpaper` exactly. What ships is 29 marks of 44, one per country that can hold one on its own ground, revealed only on the corner being read: never more than nine on screen, 18x22 units in the family own ink, and `fifty doors` demonstrated across the nine steps rather than asserted all at once. Two kilobytes, and the curve is `render.arch_path` translated rather than a fourth drawing of the one shape the aperture is cut from in three renderers. The rise is STATED, because the default is a mason flat span for an opening cut across a 900x320 drawing and rendered a rounded rectangle at glyph size. The closing sentence states that figure beside the eleven names, because a reader counting doors on the Adriatic step finds three where the card says nine and the page has to say why. It is also the commit that took the six figures OUT of the lead column, which is why the net is three rather than four: every one of them is stated twice more on this page, by the opening and by the footer, and 130 regions or 197 experiences are not this band own extent. AND 180 -> 181 WHEN THE CORNER GOT A SOCKET TO LIFT OUT OF. A lifted corner leaves a VOID and the band paints its water, so the Alpine step opened a sea-coloured channel from the Baltic to the Adriatic and the Mediterranean one cut Iberia off the continent — a false geographic statement on the one drawing whose whole argument is that geography IS the design, and the eye found it at 2x where no count could. The socket is each corner's own shape left where it was, in the land's own tone, revealed only where the group has moved off it: nine `<use>` elements cloning an inner `<g>` the lift does not touch, so it costs nine references rather than a second copy of 26 KB of country rings. IT CLONES THE INNER GROUP AND NOT `.atg`, because a clone takes whatever matches the ORIGINAL in its own position — cloning the transformed group would have moved the socket with the corner, which is the hole again with extra bytes. And it is painted by INHERITANCE, `fill` on `.atsock`, because the lit fill sits on `.atg` and a clone inherits from its own parent: paint by id and the socket lights with the corner. Measured by shooting the plate per corner with the lift frozen and live and counting pixels that were land and became water, with the page furniture and the names off because both move: 17 of 1.3 million on the Caucasus and 0 or 1 on the other eight, largest connected run 2 pixels, against 9,800 and a 2,896-pixel run with the socket removed. AND 179 -> 180 WHEN THE CORNER LEARNED TO MOVE AND THE CONTINENT STOPPED. The pan was the right repair to the wrong subject: what the reader is being shown is a CORNER, so the corner is what lifts out of the drawing and everything else holds still. A kilobyte is nine `<g>` wrappers grouping the fifty countries by their own corner and sixteen `data-lift` attributes on the names, and the grouping is not decoration — SVG has no `z-index`, probed in Chromium on the element and with `position: relative`, so document order is the only paint order there is and a corner that lifts outward has to be drawn after everything further out than it. The nine groups are emitted most-peripheral-first for exactly that, derived from each corner mean distance to the mean of every liftable country. AND 174 -> 179 WHEN THE CONTINENT LEARNED TO MOVE, which is the repair this one replaces. `.atcue` says *scroll to move through the continent* and for one commit the continent did not: only the lighting changed. Measured in Chromium at 1440 on the eastern step, the panel wash begins at x=760 of a plate running 409 to 1296 and the card own words sit over Ukraine centre at 989 482 — `elementsFromPoint` returns `.atkick` — so four of the nine corners lit ground a reader could not see. The picture pans, and a pan on a frame-filling drawing reveals the plate own background as a hard vertical seam, measured at the first attempt at x=1163. So the viewBox carries EDGE_MARGIN units of spare geography each side and `slice` crops it back to exactly the window every other measurement here is about. Five kilobytes for 340 extra units of coastline and ground beyond, which is what buys a pan with no edge in it. AND 173 -> 174 WHEN THE REGISTER BECAME THE OWNER'S STAGE. The nine corners, the drawing and the nine country lists are the bytes they were; what is new is the editorial column the brief puts inside the stage — a kicker, an intro and six derived figures — and one `<script>` element. A kilobyte for the extent of this atlas stated on the band whose subject is its extent. AND 150 -> 173 FOR THE ATLAS REGISTER, and the plate is 36 KB of the 23. Plate 05 is the band whose own sentence is `One continent. Fifty doors.` and it drew fifty names in the body serif under nine headings — a sitemap, on a page whose argument is that geography IS the design. It is now the drawn continent, sticky beside the nine corners, with each of the fifty named on its own ground and every name a door: 26 KB of country rings at a 3-unit tolerance, 7 KB of the anonymous ground beyond the atlas so the continent does not end in mid-air, 1.2 KB of names, and 119 bytes of clipPath that stops the frontier ink two units short of the data cut. It replaced 8.4 KB of fifty word-sized outlines and about 5 KB of list, so the net is 23. The ground beyond is thinned at 6 units and nothing under 200 square — four times the atlas in front of it — because it carries no name, no link and no frontier, and 11.5 KB and 7 KB are the same picture at this size, measured by rendering both. THE ALTERNATIVE WAS MEASURED AND IS WORSE: a page that says fifty doors and lists fifty names is the /themes failure word for word, and the two cheaper ways to draw fifty countries are one shared silhouette with the lit rings on top — 18.8 KB before the rings, and nine pictures of Europe with different bits lit, which is the wallpaper the aperture's own rule warns about. AND 139 -> 162 FOR TWO BANDS THE OWNER SUPPLIED A COMPOSITION FOR. Plate 03 lets its photograph leave the measure and puts the label on it, which costs a wider `sizes` and one more ladder step; plate 04 becomes two columns with the lead route drawn over a photograph of one of its own stops — one `<picture>` and one `constellation()`, the geometry of seven projected coordinates and nothing else, because a decorative squiggle would be authoring a measurement. Twenty-three kilobytes for the one thing this band is about and never drew. AND 130 -> 139 WHEN PLATE 03 STOPPED SHOWING FIVE OF NINE CORNERS. Its heading is `One destination from each corner of the continent` and `picks[:5]` drew five, so the extra four are what the heading already promised: four `<picture>` ladders at about 2.2 KB each, AVIF, WebP and JPEG at five widths. All nine corners hold a photographed, non-advisory destination, so the set the sentence names exists and the cap was the only thing in the way. AND 129 -> 130 FOR THE NINE CORNERS. Plate 05 was fifty country names A to Z in five columns — 2,527 bytes — and is the fifty grouped by this atlas's own nine macro regions, 3,552. A kilobyte for nine corner headings, nine links to nine macro pages, and the hoisted clause saying which 41 of the fifty the picture at the top of the page opens and why the other nine are names: six too small to draw at 6 units and three carrying a travel advisory. Both figures derived from `living_atlas`, because the band above and the band below had differed by nine for the life of the plate sequence with nothing on the page saying so. AND 147 -> 129 WHEN THE HOMEPAGE STOPPED SHIPPING A CONTINENT IT NEVER DREW. `constel_defs()` inlines one thinned lod0 silhouette so that thirteen glyphs cost ONE coastline, and the homepage referenced neither id it provides — it draws its hero from `#heroland` and its own `#lz-*` rings. 18,806 bytes, 12% of the most-visited page on the site, for the life of the plate sequence. Sixteen pages had it: /themes at 37%, /experiences, /plan, nine macro pages and three motion pages, about 301 KB in all. Nineteen call sites emit the defs and TWO of them guard the call, which is the fourteen-call-sites-forgot-the-motif shape exactly — so the strip is in `page()`, where a caller cannot get half of it right because a caller no longer does any of it, and `c_unused_geometry` fails on any page that emits the block and clones it zero times. A ceiling going DOWN is a saving being locked in: this number exists because a page once shipped 90 KB of coastline under a map and every gate stayed green. AND 144 -> 145 WHEN THE FRAME GOT ITS REFINEMENT PASS. The footer is on every page, so every number in it is 1,032 copies: bigger links, a rule under each column head, an outward mark on each of the six external services, the brand statement split into a display line and a note, the services disclosure moved out of a column into its own band with a label, and the legal strip split into two levels. One kilobyte of markup for all of it, because the air is CSS and only the arrows, the two spans and the band are bytes. It is on this row as well as on max_page_kb because the homepage is the page this ceiling watches and a site-wide component is exactly the thing it cannot see coming."
                       ...
                       "  152 -> 148 WHEN THE HERO BECAME THE LIVING ATLAS. It reads as a saving and it is a swap: the ground beyond the continent, the non-atlas neighbours, the one country the 52°E cut runs through and both data-cut fades came off — about 25 KB — and forty-one clipped photographs went on for about 9. The pictures themselves are files a reader fetches rather than bytes in this document, and the clip is a `<use>` of the land path the drawing already carries rather than a second copy of 43 KB of country rings. What this ceiling cannot see is that the page now asks for 41 photographs at the 480 step, which is about 950 KB of images: the ladder's smallest rung is 480 and most of these countries render under 150px, so a sixth rung would halve it. Recorded rather than answered here, because a smaller step is a decision about every purpose on the site.\n  148 -> 151 THE ATLAS INDEX DRAWS THE FIFTY COUNTRIES IT LISTS. Plate 05 is the band whose own sentence is ‘One continent. Fifty doors.’ and it listed fifty names in the body serif under nine headings: a sitemap, on a page whose argument is that geography IS the design. Every one of them has an outline in data/geo/ and the band showed none — the /themes failure word for word. country_mark is country_door at the size of a word: the same own-frame fitting, the same floor deciding which six are a ringed point, and no photograph, so it needs no register row and asks for no image file. 8.4 KB of path data for the whole set, against 628 KB for the doors and 18.8 KB for one shared continental silhouette that would still need the lit rings on top of it. It is the cheapest of the three ways to draw fifty countries and the only one that draws fifty different shapes rather than nine pictures of Europe with different bits lit. \n  AND THAT BAND IS GONE, WHICH MAKES THIS PARAGRAPH THE THING IT WARNS ABOUT. Plate 05 became the nine-corner stage — scrolling to a corner lights that corner's countries on the continent itself, so the shapes are drawn at the scale the picture is at rather than at the size of a word — and the predecessor was left standing: the builder composed the string on every build and threw it away, country_mark had exactly one caller and it was that one, and about eighteen stylesheet rules matched a class `grep -rl dcorner site --include=index.html` could not find on any page. So the 8.4 KB this row credits for the 148 -> 151 move is not in the document and has not been for several commits. A comment claiming evidence is read as evidence, and a reason in the register is the worst place for it: the register is what a person reads to find out why a number moved. \n  AND 192 -> 199 FOR THE GRATICULE THE HERO HAD BEEN BUILDING AND THROWING AWAY. `heroeurope()` called `graticule_layer()`, assigned it to a local and never interpolated it, under a paragraph arguing that under a conic a parallel is a circular arc about the cone apex so the grid is what SHOWS the projection this product draws on. 6.3 KB, fifteen paths, under the land where /map puts it so it shows on water and never crosses the parchment. Four strengths were rendered and looked at; 34% of the coast ink against this drawing's white wall, where /map's 14% is against near-black. The per-pixel ratio is 1.03:1 and the line is unmistakable in the render, because a ratio is a claim about a patch and a five-hundred-pixel curve is not one."},
            "weight.max_page_kb": {
                "value": round(max(len(b) for b in bodies.values()) / 1024),
                "kind": "ceiling",
                "why": "601 -> 606 WHEN THE INSTRUMENT STOPPED ENDING EUROPE ON A RULED LINE. /map drew the continent to the 45 degree meridian and then painted a sea-coloured ramp over the open water east of it, and the two are one fault: the fade is anchored on the DATASET cut at 52 degrees while `Projection.path` clips every ring to the PROJECTION extent at 45, so the ramp lay almost entirely east of the edge it exists to cover. Measured in Chromium at 1440, land to x=731 at y=100 and the ramp from x=684 to 783; at y=400, land to 884 and the ramp 876 to 974. A drawing that ends on a meridian ends on a straight line whatever the meridian is, so the answer is the atlas register own: the anonymous ground beyond drawn in the same tone, and then there is no seam and the fade comes off entirely. 4,970 bytes of `beyondmass` thinned at 6 units with nothing under 200 square, plus 930 bytes of clipPath that stops the frontier ink at the extent, against the two gradients and two rects the fade cost. Verified on the pixels after: land runs to x=999 on every sampled row and there is no transition anywhere. AND 586 -> 601 WHEN THE INSTRUMENT GOT A GROUND IT CAN TURN ON, WHICH IS THE ONE MOVE ON THIS ROW WHERE THE BYTES BUY A LAYER MOST READERS WILL NEVER SWITCH ON. 15,725 bytes of hypsometric bands at plate 05's own settings, kept only where this drawing draws ground — the first version was relief_wash raw and painted Greenland's mountains in the sea above Iceland, which every count reported as correct and a rendered contact sheet found in one look. It ships in the document rather than being fetched: this page loads no geometry over the network at all, so the alternative is a request and a second index for a layer that is 2.6% of a page already carrying 253 KB of per-destination photograph fields, and inlining keeps the no-JavaScript case honest because the data is in the page and the reader simply has not asked for it. AND 581 -> 598 WHEN /map BECAME AN ATLAS. It drew no water, no rivers, no coast and no type — not one `lyr-*` layer and zero `<text>` elements, on the page titled *Europe, and everything we hold in it*. Seventeen kilobytes buys the water, thirty-five rivers and the big lakes, a graticule, thirteen country names and six sea names, every one of them from a helper that already existed and had never been on this page. A separate coast pass is refused on its bytes and the refusal is written down. The heaviest page on the site. Guards against a template "
            ...
            "map.relief_pages": {
                ...
                "kind": "floor",
                "why": "AND 285 -> 286 WHEN THE INSTRUMENT GOT A GROUND IT CAN TURN ON. /map drew rivers, lakes, a graticule, six sea names, twenty-four country names and fifty interactive country shapes, and never the height of the land under them: the Alps and the Hungarian plain were the same stone on the page whose subject is the continent. `c_terrain`'s first promise refuses relief on a map that declares itself an instrument, and the reason survives re-reading — a reader reads VALUES off an instrument, so a wash nobody asked for reads as a data layer rather than as ground. It is a reason about relief being the GROUND, and this map already has four geography layers a reader switches, so the fifth answers the objection rather than overruling it: off by default, marked hidden AND carrying the display presentation attribute so the group is inert with JavaScript off too, named in the key by all three of its band colours, and the reader's own choice. The frontier underneath it is kept by arithmetic rather than by a class — a boundary here is a stroke on the land path and a stroke-only pass above the bands is a third copy of the geometry at 43 KB, refused on this page by name — so the bands MULTIPLY: measured on the shipped hexes the frontier reads 3.39:1 on bare land, 1.90:1 under a band composited normally and 3.16:1 under the same band multiplied. Pages that draw hypsometric relief. Every one of them "
                       ...
                       "AND 226 -> 262 WHEN THE PLACE PAGE STOPPED THROWING ITS DRAWING AWAY. A place page opened on `photograph if has_photo else minimap(...)` — an either/or — while its own comment twenty lines down says THE OPENING KEEPS THE MAP and *the strip is where the photographs go*. It was invisible while no `place:` purpose was filled; two photograph batches merged and forty place pages lost their geography entirely, relief included, along with the caption saying there is no honest map of a building but there is an honest map of where the building is. `head_figure`/`moved_drawing` is the site's own contract and this was its fourth caller with a private if/else: the photograph keeps the opening `place-hero` declares and the drawing moves to a band below. The figure is higher than the 242 this branch had because a photographed place page now carries BOTH, where before it carried one. Nothing was acquired for it. AND 262 -> 285 WHEN THE COUNTRY PORTRAITS STOPPED BEING THE ONLY FAMILY THAT NAMES THINGS WITH NO RELIEF. Not one of the fifty drew a band, so the second-largest family here — and the page a reader arrives on from a search for \"Austria travel\" — drew its land as one flat tone, Switzerland, Norway and Iceland with it. The gate is the file's OWN two thresholds (min_spread 300, min_crest 600) applied to the highest of the country's own destinations' [spread, crest] records, plus `pages.PORTRAIT_RIDGE_M`, the lowest band boundary a ridge is drawn on: 23 of the fifty qualify, 17 are refused by the ground and Norway and Turkiye by the 1,500 km frame cap. It is a RIDGE rather than a wash because the highlight and the relief are the same variable — measured with the plates' opaque fills over the subject, Switzerland's painted subject:neighbour step fell from 1.371 to 1.022 and every band came out darker than its neighbours, so the plate stopped saying which country the page is about; a 30% wash is worse rather than better at 1.213 with the bands' own steps at 1.019. Stroke only leaves the fill hierarchy untouched. "},
            "signature.apertures": {
