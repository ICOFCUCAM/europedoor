...
/* THE FIXTURES ARE READ FROM THE REGISTRY, NEVER TYPED.
 *
 * Four tests in this file named `door-coast` and `door-food`, and both
 * purposes were deleted the day the four homepage doors became the plate
 * sequence. `specOf()` then refused them at entry ONE — so the assertions
 * about entry TWO, about a surface ticked twice and about one photograph on
 * two surfaces were all pre-empted by a refusal none of them was written
 * for, and a suite whose subject is the batch boundary stopped reaching the
 * batch boundary. It reported four honest failures naming the wrong thing.
 *
 * `tools/photo-tests.py` had the same fixtures and was repaired in the same
 * commit that deleted the purposes; this file was not looked at. *A rule
 * stated once and applied to one of its call sites* — and the general form
 * of it is that a fixture naming a purpose is a SECOND declaration of which
 * purposes exist, kept in a test file where nothing regenerates it.
 *
 * So the two come out of `desk/registry.json`, which is the same document
 * `specOf()` consults, is generated by `tools/desk-registry.py`, and fails
 * CI when it is stale. A fixture drawn from it cannot be a purpose the
 * route refuses, whatever anybody deletes next. The first two authored
 * purposes are taken rather than a chosen pair, because naming a pair here
 * is how this file got into trouble. */
const REG = JSON.parse(fs.readFileSync(
  path.join(ROOT, "desk", "registry.json"), "utf8"));
const [PURPOSE_A, PURPOSE_B] = REG.purposes.slice(0, 2).map((r) => r.purpose);
assert.ok(PURPOSE_A && PURPOSE_B && PURPOSE_A !== PURPOSE_B,
          "the registry must declare at least two distinct purposes for the "
          + "batch fixtures to be about a batch at all");
...
await t("a batch refuses one bad entry and dispatches nothing", async () => {
  ...
  const r = await acq({ provider: "pexels", batch: [
    { purpose: PURPOSE_A, photo_id: "111", alt: "a long enough description" },
    { purpose: PURPOSE_B, photo_id: "not-an-id", alt: "another description" },
  ] });
  assert.strictEqual(r.statusCode, 400);
...
await t("one surface may not be ticked twice in a batch", async () => {
  ...
  const r = await acq({ provider: "pexels", batch: [
    { purpose: PURPOSE_A, photo_id: "111", alt: "a long enough description" },
    { purpose: PURPOSE_A, photo_id: "222", alt: "another description" },
  ] });
  assert.strictEqual(r.statusCode, 400);
...
await t("one photograph may not be ticked for two surfaces in a batch",
  async () => {
    ...
    const r = await acq({ provider: "pexels", batch: [
      { purpose: PURPOSE_A, photo_id: "111", alt: "a long enough description" },
      { purpose: PURPOSE_B, photo_id: "111", alt: "another description" },
    ] });
    assert.strictEqual(r.statusCode, 400);
...
await t("a valid batch dispatches the batch stage with every id", async () => {
  ...
  const r = await acq({ provider: "pexels", batch: [
    { purpose: PURPOSE_A, photo_id: "111", alt: "a long enough description" },
    { purpose: PURPOSE_B, photo_id: "222", alt: "another long description" },
  ] });
