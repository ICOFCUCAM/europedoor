 ...
 * this process — every preview goes through /api/thumb, so the editor's
 * machine makes no request to pexels.com at all.
 *
...
(function () {
  ...
  var REG = { purposes: [], slots: [] };
  var CHOSEN = null;           // the candidate in the acquire dialogue
  var POLL = null;
  ...
  /* ── the registry, and the requirements that come from the slot ── */
  ...
  function onSlot() {
    ...
    var spec = isSlot ? rows[0] : REG.purposes.filter(function (p) {
      return p.purpose === v;
    })[0];
    showBrief(spec);
    showNeeds(spec);
  ...
  function draw(res, purpose) {
    ...
    el("sheet").innerHTML = res.candidates.map(function (c) {
      return '<article class="cand">' +
        ...
        '<p class="dims">' + c.width + " × " + c.height + " · " + c.aspect + " · id " + esc(c.id) + "</p>" +
        (c.suits
          ...
          : '<p class="verdict no">Does not meet this slot<ul><li>' +
            ...
            "</li></ul></p>") +
        (c.already.length
          ? '<p class="dupe">Already registered, for ' + c.already.map(esc).join(", ") + "</p>"
          : "") +
        '<p class="acts">' +
  ...
  function watch(jid) {
    ...
    tick(jid);
    POLL = setInterval(function () { tick(jid); }, 1200);
  }

  function tick(jid) {
    api("/api/job/" + encodeURIComponent(jid)).then(function (r) {
      if (!r.ok) return;
      var job = r.j;
      el("prog-steps").innerHTML = job.steps.map(function (s) {
        var mark = s.state === "done" ? "✓" : s.state === "failed" ? "✕" : "·";
        return '<li class="' + s.state + '"><span class="tick">' + mark +
          '</span><span class="lbl">' + esc(s.label) + "</span>" +
          (s.detail && s.state !== "failed"
            ? '<span class="det">' + esc(s.detail.split("\n").pop()) + "</span>"
            : "") + "</li>";
      }).join("");
      if (job.state === "done") {
        stop();
        el("prog-title").innerHTML = "Committed to <span class=\"branch\">" +
          esc(job.branch) + "</span>";
        loadRegistry();
      } else if (job.state === "failed") {
        stop();
        /* NEVER SILENTLY CONTINUE. The step that failed, what it said, and
           the fact that nothing was published — which is true, because the
           commit is the last step and a failure before it leaves the working
           tree for you to inspect. */
        el("prog-title").textContent = "Acquisition failed";
        el("prog-why").hidden = false;
        el("prog-why").textContent =
          (job.failure || "") +
          "\n\nNo branch was made. Nothing was published.";
      }
    });
  }
  ...
  function fillLibrary() {
    ...
    /* A LIBRARY OF 593 ROWS IS A SCROLLBAR. The filled ones come first
       because they are what a library is for; the empty ones are a work
       list and are capped. THE COUNT SAYS WHAT IS ON THE SCREEN — the first
       version said "Showing 593" while listing 60, which is the failure this
       product already records one level up: a number that is not the set's
       own extent reads as one. */
    ...
    el("lib").innerHTML = showing.map(function (p) {
      return '<div class="slotrow"><span class="pill ' + p.status + '">' +
        ...
        '<p class="meta">' + (p.photograph
          ? esc(p.photograph.photographer) + "<br>" + esc((p.photograph.sha256 || "").slice(0, 16))
          : p.min_width + "px · " + p.min_aspect + "–" + p.max_aspect) +
        "</p></div>";
    }).join("") + (tail.length > 60
