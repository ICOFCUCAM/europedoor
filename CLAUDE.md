...
| doing | read |
|---|---|
| **the mandate: what a first-class gateway to Europe would be, and where this one is not** | **`docs/first-class-audit.md`** — 27 surfaces rendered at 1280 and 390 and then measured. Three findings: on a phone 21 of 22 surfaces show a visitor nothing but type; the site has one composition rendered 1,033 times; the homepage decays monotonically after the hero. The eight benchmark sites are BLOCKED by the egress proxy and the benchmark half is labelled second-hand |
